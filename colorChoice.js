...
function colorChange(event) { 
    ...
    let clickedColor=event.target.dataset.currentColor;      
  
    let shapes=document.getElementsByClassName('shape');
    console.log(shapes);
    let currentColor=document.getElementById('central').dataset.currentColor;

    for (var i = 0; i < shapes.length; i++) {
      shapes[i].classList.add(clickedColor);
      shapes[i].classList.remove(currentColor);
    }

    event.target.classList.add(currentColor);
    event.target.classList.remove(clickedColor);

    event.target.dataset.currentColor = currentColor;
    document.getElementById('central').dataset.currentColor = clickedColor;
